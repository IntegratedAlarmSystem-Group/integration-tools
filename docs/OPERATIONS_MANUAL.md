...
The ALMA Observatory consists of many hardware and software systems which are continuously generating data and its related operations depend on the availability of high-level, and easy-to-understand information about the status for these multiple systems.
...
Also, the operators can perform actions as the *acknowledgement* or *shelving* of alarms.

Other user groups are able to access the displays, with actions allowed according to specific permissions. The description of the authentication and authorization system for the IAS can be found at the end of this document.
...
The alarms organized in the displays of the system, use the representation exposed in the following legend, according to different properties.

![Alarms Legend](./images/legend.png)

First, we have the *clear* and *set* status, used when an alarm is resolved or if a condition configured in the system for the related device is not satisfied, respectively. The mark related to each status is shown in green, for the *clear* value, and red or yellow, for the *set* value. The usage of the colors red and yellow depends on the priority level of the alarm.
...
An alarm can also have dependencies related to other alarms according to a hierarchy, for example, when monitoring points are related to a selected device and its subcomponents.
...
| Icon  | Action  |
|---|---|
| ![Ack](./images/ack-on.png) | ack  |
| ![Shelve](./images/shelve.png)  | shelve  |
| ![Go to wiki]()  | go to documentation  |

## Acknowledgment

The operators are requested to use the *ack* action each time an alarm is active.

![Ack Form](./images/ack_form.png)
...
![Ack Tree View](./images/ack_tree_view.png)
...
![Shelve Form](./images/shelve_form.png)
...
![Weather Selected](./images/weather_selected.png)
...
![Antennas Selected](./images/antennas_selected.png)
